...
function classify(body) {
  ...
  */
  ...
  /*
    「내 정보」는 다른 모든 규칙보다 앞이다.

    fullText 는 사람이 적은 말 앞에 «매칭된 블록 이름»(intent.name)을 붙인다. 그래서 카카오가
    「내정보」를 «하늘땅대시보드» 블록으로 흘려보내면 분류할 글자가 "하늘땅대시보드 내정보"가
    되고, 맨 아래 /현황|대시보드|요약|오늘/ 에 걸려 오늘 현황이 돌아왔다. 등록된 뒤에는
    식별키를 볼 다른 길이 없으니 명령이 통째로 죽는다.
...
async function fetchOwnervistaDailySales(since, until) {
  ...
  if (response.status === 404) {
    /*
      **404 하나에 원인이 셋이다.** 오너비스타는 남이 두드리는 것을 막으려고 「없는
      매장」·「틀린 토큰」·「꺼진 주소」를 **전부 404 로** 답한다. 우리는 그중 어느
      것인지 알 수 없다.

      한때 이 자리에서 「만료됐습니다」라고 단정했다. 그래서 **아직 주소를 만들지도
      않았는데 «새 주소를 받으라»고 시켰고**, 사장님은 시킨 대로 갈아 끼우셨다 —
      고쳐지지 않는 일을 하게 만든 것이다.

      모르는 것은 모른다고 말하고, **확인할 순서**를 대신 알려 준다.
    */
    return {
      status: "config_required",
      message: "오너비스타가 이 주소를 못 알아봅니다. ①오너비스타 매출 화면에 주소가 "
        + "만들어져 있는지 ②시크릿에 마지막 칸(토큰)만 들어갔는지 ③토큰을 바꾼 뒤 "
        + "함수를 다시 배포했는지 확인해 주세요.",
    };
  }
  if (!response.ok) {
