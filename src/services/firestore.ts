...
export function subscribeGranterSyncRuns(cb: (v: GranterSyncRun[]) => void, onError: ErrCb): Unsub {
  return subscribe(
    ...
    (d, id) => ({
      ...
      cardImportedCount: Number(d.cardImportedCount ?? 0),
      cardUpdatedCount: Number(d.cardUpdatedCount ?? 0),
      accountImportedCount: Number(d.accountImportedCount ?? 0),
      accountUpdatedCount: Number(d.accountUpdatedCount ?? 0),
      ignoredCount: Number(d.ignoredCount ?? 0),
      rangeStart: String(d.rangeStart ?? ""),
      rangeEnd: String(d.rangeEnd ?? ""),
      message: d.message,
    }),
