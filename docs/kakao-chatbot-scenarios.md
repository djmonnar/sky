...
날짜 · 시간 · 인원 · 이름 · 전화번호(선택) 순서로 적으면 명령어 없이도 예약이 잡힙니다.
...
내일 저녁 7시 4명 홍길동 창가
9/5 18시 3명 박현제                ← 전화 없이도 됩니다 (연락처: 없음)
```

- 시간에 오전/오후가 없고 1~9시면 저녁으로 봅니다 (6시 → 18:00). 10·11·12시는 그대로.
- 전화번호는 010을 빼고 8자리만 적어도 됩니다. 안 적으면 빈칸으로 둡니다.
- 같은 형식에 `취소`를 붙이면 취소됩니다. 지우지 않고 상태만 「취소」로 바꿔서 예약 관리에 이력이 남습니다.

```text
9/5 박현제 취소
9/5 18시 박현제 취소               ← 같은 이름이 여럿이면 시간으로 고릅니다
```

배포 없이 파서를 점검하려면 `cd functions && npm run test:kakao`.
...
예약 등록 홍길동 010-1234-5678 내일 오후 7:30 4명 창가
...
공지 수정 123 오늘 단체 예약 많으니 세팅 다시 확인
공지 삭제 123
전달사항 등록 주방 재료 입고 확인 필요
전달사항 수정 123 재료 입고 확인 완료
전달사항 삭제 123
